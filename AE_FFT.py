# -*- coding: utf-8 -*-
"""
Created on Sun Dec 19 21:31:47 2021
...
def plot_AE_event(time_history):
    ...
    color = 'tab:red'
    ax1.set_xlabel('time (count)')
    ax1.set_ylabel('voltage [V]', color=color)
    ax1.plot(np.array(time_history), color=color) 
...
def plot_AE_slope(record_time_list,RMS_cum_list):
    ...
    for i in range(len(time_list)):
        if time_list[i] > 500 and time_list[i] < 650:
    ...
    color = 'tab:red'
    ax1.set_xlabel('time [s]')
    ax1.set_ylabel('slope [V/s]', color=color)
    ax1.plot(np.array(time_list),np.array(slope_list), color=color) 
    ax1.tick_params(axis='y', labelcolor=color)
    #plt.ylim(-4,4)
...
def plot_strain_slope(strain_list,stress_list):
    ...
    for i in range(len(new_strain_list)):
        if new_strain_list[i] > 1.0 and new_strain_list[i] < 1.5:
...
#plot cumulative RMS with respect to time
def main():
    #RMS_list = []
    first_arrival_list = []
    real_record_time_list = []
    record_time_list = []
    data_file = ['1','2','3','4','5','6','7','8']
    time_last = 0
    AE_dataset = []
    full_AE_dataset = []
    for file in data_file:
        print("Read file"+file)
        number_of_AE,record_time,AE_list = read_AE_file("C:/Users/huxxx/OneDrive/Desktop/SiC/SiC real specimen testing/11_12_2021/" + file + ".txt")
        ...
        #get first arrival and data sample after first arrival 
        for i in range(number_of_AE):
            first_arrival = calculate_first_arrival(AE_list[i])
            first_arrival_list.append(first_arrival)
        
            if first_arrival !=0:
                AE_dataset.append(AE_list[i][first_arrival:])
                full_AE_dataset.append(AE_list[i])
    
    timestep = 0.05    
    for amplitude in AE_dataset[6000:6010]:
        
        count_index = 6000
        real_amplitude = full_AE_dataset[count_index]
        count_index = count_index + 1
        
        amplitude = amplitude[0:2000]
        ...
        data1.plot(real_index,real_amplitude, color='red', label='Amplitude')
        plt.legend()
        plt.minorticks_on()
        #plt.xlim(0., 10.)
    ...
    #print(len(first_arrival_list))
    #print(len(record_time_list))
    #print(RMS_list)
    #print(np.cumsum(RMS_list))
    ...
    stress_file = open("C:/Users/huxxx/OneDrive/Desktop/SiC/SiC real specimen testing/11_12_2021/stress.txt",'r')
    ...
    for line in lines:
        ...
        #print(minute,second)
        real_time = round(time + 806.248+1019.66)
    ...
    estimate_time = real_record_time_list[2431]
    print(estimate_time)
    ...
    print(axial_stress_list[1402],hoop_stress_list[1402])
